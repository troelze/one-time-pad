...
* Description: This program connects to otp_enc_d, and asks it to perform a one-time pad style encryption.
...
int main(int argc, char *argv[])
{
    ...
    //check if bad characters in KEY, count number of chars, and make sure files are valid!
    FILE *KEY_verify;
    char c1;
    ...
    fclose(KEY_verify);
    //check if bad characters in PT, count number of chars, and make sure files are valid!
    FILE *PT_verify;
    char c;
    ...
    if (strncmp(verify, "valid", 5) != 0)
    {
        fprintf(stderr,"You do not have access.\n", argv[3]); 
        exit(2); // Check 
    }
    ...
    close(plaintextFD);
    write(socketFD, "&", 1); //write ampersand to delineate plain text from keyfile
    ...
    //print cyphered text received from otp_enc_d to stdout
    int read_return;
    do {
